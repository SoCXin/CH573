...
void send_led_color_indicate(struct indicate_param *param)
{
    ...
    net_buf_simple_add_le16(&(ind->buf->b), ALI_GEN_ATTR_TYPE_RGBCOLOR);

    /* Add led_color status  */
    read_led_color(&color_R, &color_G, &color_B);
    net_buf_simple_add_le32(&(ind->buf->b), color_R);
    net_buf_simple_add_le32(&(ind->buf->b), color_G);
    net_buf_simple_add_le32(&(ind->buf->b), color_B);
