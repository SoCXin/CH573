...
/* flash的数据临时存储 */
__attribute__((aligned(8))) uint8 block_buf[4];
...
void SwitchImageFlag(UINT8 new_flag)
{
	...
	/* 读取第一块 */
	EEPROM_READ( OTA_DATAFLASH_ADD, block_buf, FLASH_BLOCK_SIZE );
	
	/* 擦除第一块 */
	EEPROM_ERASE(OTA_DATAFLASH_ADD,FLASH_BLOCK_SIZE);

	/* 更新Image信息 */
	block_buf[0] = new_flag;
	
	/* 编程DataFlash */
	EEPROM_WRITE(OTA_DATAFLASH_ADD, (UINT32 *)block_buf, FLASH_BLOCK_SIZE);
	
	/* 打印输出消息 */
	EEPROM_READ( OTA_DATAFLASH_ADD, &ver_flag, 1 );
...
void Rec_OTA_IAP_DataDeal(void)
{
    switch(iap_rec_data.other.buf[0])
    {
        ...
        OTA_IAP_SendCMDDealSta(status);
            ...
            for(i = 0; i < 4; i++)
            {
              block_buf[OpAdd + i] = iap_rec_data.program.buf[i];
            }
				  }
					status = SUCCESS;
					OTA_IAP_SendCMDDealSta(status);
        ...
        VerifyStatus |= status;
			...
			else
			{
				...
				if( (OpAdd + OpParaDataLen) <= FLASH_BLOCK_SIZE )
				{
				  /* 前四字节和RAM保存的参数对比，其余直接返回成功 */
				  if(OpAdd==0)
				  {
            for(i=0; i<4; i++)
            {
              if(block_buf[OpAdd+i] != iap_rec_data.verify.buf[i])break;
            }
            if(i == 4) status = SUCCESS;
            else        status = 0xff;
				  }
				  else
				    status = SUCCESS;
		...
		case CMD_IAP_END:
		{
			...
			else
			{
				...
				FLASH_ROM_WRITE(IMAGE_A_START_ADD, (PUINT32) block_buf, 4);
