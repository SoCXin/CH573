...
// HID Report callback
typedef uint8
(*hidDevReportCB_t)( uint8 id, uint8 type, uint16 uuid, uint8 oper, uint16 *pLen, uint8 *pData );

// HID event callback
typedef void
(*hidDevEvtCB_t)( uint8 evt );

// HID passcode callback
typedef void
(*hidDevPasscodeCB_t)( uint8 *deviceAddr, uint16 connectionHandle, uint8 uiInputs, uint8 uiOutputs );
 ...
 */
extern uint16
HidDev_ProcessEvent( uint8 task_id, uint16 events );
 ...
 */
extern void
HidDev_Register( hidDevCfg_t *pCfg, hidDevCB_t *pCBs );
 ...
 */
extern void
HidDev_RegisterReports( uint8 numReports, hidRptMap_t *pRpt );
 ...
 */
extern void
HidDev_Report( uint8 id, uint8 type, uint8 len, uint8 *pData );
 ...
 */
extern bStatus_t
HidDev_SetParameter( uint8 param, uint8 len, void *pValue );
 ...
 */
extern bStatus_t
HidDev_GetParameter( uint8 param, void *pValue );
 ...
 */
extern void
HidDev_PasscodeRsp( uint8 status, uint32 passcode );
 ...
 */
extern bStatus_t
HidDev_ReadAttrCB( uint16 connHandle, gattAttribute_t *pAttr, uint8 *pValue, uint16 *pLen, uint16 offset, uint16 maxLen,
                   uint8 method );
 ...
 */
extern bStatus_t
HidDev_WriteAttrCB( uint16 connHandle, gattAttribute_t *pAttr, uint8 *pValue, uint16 len, uint16 offset, uint8 method );
