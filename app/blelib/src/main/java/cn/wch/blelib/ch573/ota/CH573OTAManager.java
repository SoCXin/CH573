package cn.wch.blelib.ch573.ota;
...
import cn.wch.blelib.ch573.constant.Constant;
import cn.wch.blelib.ch573.ota.callback.IProgress;
import cn.wch.blelib.ch573.ota.command.CommandUtil;
...
import cn.wch.blelib.ch573.ota.util.DebugUtil;
import cn.wch.blelib.ch573.ota.util.FileParseUtil;
import cn.wch.blelib.ch573.ota.util.FormatUtil;
import cn.wch.blelib.ch573.ota.util.ParseUtil;
 ...
 */
public class CH573OTAManager {
    private Application application;
    private static CH573OTAManager ch573OTAManager;
    ...
    public static CH573OTAManager getInstance(){
        if(ch573OTAManager==null){
            synchronized (CH573OTAManager.class){
                ch573OTAManager=new CH573OTAManager();
            }
        }
        return ch573OTAManager;
    }
    ...
    public int getHexFileEraseAddr(@NonNull File file) throws CH573OTAException{
        if(file.getName().endsWith(".hex") || file.getName().endsWith(".HEX")){
            return FileParseUtil.parseHexFileStartAddr(file);
        }else {
            throw new CH573OTAException("only support hex file");
        }
    }
     ...
     * @param progress 回调
     * @throws IOException
     * @throws CH573OTAException
     */
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public void start(int eraseAddr,@NonNull File file, @NonNull CurrentImageInfo currentImageInfo, IProgress progress) throws IOException, CH573OTAException {
        stopFlag=false;
        ...
        if(file.getName().endsWith(".bin") || file.getName().endsWith(".BIN")){
            ...
            throw new CH573OTAException("CH57X only support hex and bin image file");
        }
        if(byteBuffer==null){
            throw new CH573OTAException("parse file fail");
        }
        ...
        if(!checkImageIllegal(currentImageInfo,byteBuffer)){
            throw new CH573OTAException("image file is illegal!");
        }
    ...
    @Deprecated
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public void start(@NonNull File file, @NonNull CurrentImageInfo currentImageInfo, IProgress progress) throws IOException, CH573OTAException {
        stopFlag=false;
        this.progress=progress;
        //image信息
        LogUtil.d("开始升级固件： "+file.getAbsolutePath());
